...
// server-side anti-cheat: every submitted run is REPLAYED against the real maze — each step
// must cross an actual open corridor, never touch a spike, start at the start, and end at the
// goal — and its time must clear a plausible per-cell floor. The maze data is generated from
// the same Swift generator the app uses (levels.js), so client and server agree exactly.
...
// Each level's grid size (matches the Swift LevelSpec table). v1 anti-cheat is lightweight
// (blueprint §4): trail structure + Manhattan-length floor + per-cell time floor + trail hash.
// Full server-side maze replay (reject any step that crosses a wall) is the documented next
// step — see server/README; the data is generated by `DUMP_LEVELS=1 swift test`.
const LEVELS = {
  1:[5,5], 2:[6,6], 3:[6,7], 4:[7,8], 5:[7,8], 6:[8,8], 7:[8,9], 8:[8,9], 9:[9,9], 10:[9,10],
  11:[9,10], 12:[10,10], 13:[10,11], 14:[10,11], 15:[11,11], 16:[11,11], 17:[12,12], 18:[12,13],
  19:[12,13], 20:[13,13], 21:[13,14],
};
...
/// Validate a trail is a legal corridor walk (structure only) and return its step count, or
/// throw HttpError. v1 doesn't replay walls server-side, so it enforces the strongest bounds
/// available without the maze: in-bounds, contiguous, start→goal, and length ≥ Manhattan.
export function validateTrail(trail, w, h) {
  ...
  for (let i = 1; i < trail.length; i++) {
    const d = Math.abs(trail[i][0] - trail[i - 1][0]) + Math.abs(trail[i][1] - trail[i - 1][1]);
    if (d !== 1) throw new HttpError(422, "trail_not_contiguous");
  }
  const steps = trail.length - 1;
  if (steps < (w - 1) + (h - 1)) throw new HttpError(422, "trail_too_short");
  return steps;
}
...
async function hScore(req, env, player) {
  ...
  const dims = LEVELS[levelId];
  if (!dims) return fail(400, "bad_level");
  ...
  let steps;
  try { steps = validateTrail(trail, dims[0], dims[1]); }
...
async function hHealth(req, env) {
  ...
  return ok({ ok: !!r, levels: LEVEL_COUNT });
}
...
export { LEVELS, HARD_MIN_MS_PER_CELL, SOFT_MIN_MS_PER_CELL };
